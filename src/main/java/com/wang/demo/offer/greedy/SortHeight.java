...
public class SortHeight {
    ...
    public static List<Pair<Integer, Integer>> solution(List<Pair<Integer, Integer>> people) {
        ...
        for (int i = 0; i < people.size(); i++) {
            ...
            processPairIndex(people.get(i), pair.getValue(), result);
        }
        return result;
    }

    private static void processPairIndex(Pair<Integer, Integer> pair, int index, List<Pair<Integer, Integer>> tmpList) {
        ...
        if (Objects.isNull(oldPair)) {
            ...
            if (oldPair.getValue() > pair.getValue() || oldPair.getKey() > pair.getKey()) {
                processPairIndex(oldPair, index + 1, tmpList);
                tmpList.set(index, pair);
            } else {
                processPairIndex(pair, index + 1, tmpList);
            }
        }
    }
