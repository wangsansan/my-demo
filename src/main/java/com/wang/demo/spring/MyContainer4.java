...
//    private static Map<Class, Object> NON_FIELD_BEAN_MAP = new HashMap<>();
    ...
    private static Map<Class, Supplier<Object>> BEAN_SUPPLIER = new HashMap<>();
...
//        o = NON_FIELD_BEAN_MAP.get(clazz.getName());
//        if (Objects.nonNull(o)) {
//            return o;
//        }
        Supplier<Object> supplier = BEAN_SUPPLIER.get(clazz);
        if (Objects.nonNull(supplier)) {
            o = supplier.get();
//            NON_FIELD_BEAN_MAP.putIfAbsent(clazz, o);
            BEAN_MAP.putIfAbsent(clazz, o);
        }
        return o;
    }
...
//        NON_FIELD_BEAN_MAP.entrySet().removeIf(it -> clazzSet.contains(it.getKey()));
    ...
    private static Object complete(Class clazz) throws Exception {
        ...
        if (instance == proxyed) {
//            if (!NON_FIELD_BEAN_MAP.containsKey(metaNode.getClazz())) {
//                throw new Exception(String.format("clazz %s have been aop proxyed more than once", clazz));
//            } else {
//                proxyed = NON_FIELD_BEAN_MAP.remove(metaNode.getClazz());
//            }
            if (!BEAN_MAP.containsKey(metaNode.getClazz())) {
                throw new Exception(String.format("clazz %s have been aop proxyed more than once", clazz));
            } else {
                proxyed = BEAN_MAP.remove(metaNode.getClazz());
            }
        }
        ...
        BEAN_MAP.putIfAbsent(metaNode.getClazz(), proxyed);
        return proxyed;
    }
